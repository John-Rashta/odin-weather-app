...
export default function setupDom() {
  ...
  const changeTemperature = function changeTempToUSorMetric(location, measure) {
    ...
    firstDiv[0].textContent =
      location.days[0][`tempMin${measure}`] + finalString;
    firstDiv[1].textContent = location.days[0][`temp${measure}`] + finalString;
    firstDiv[2].textContent =
      location.days[0][`tempMax${measure}`] + finalString;
    ...
    Array.from(restChildren).forEach((day, index) => {
      ...
      currentDiv[0].textContent =
        location.days[index + 1][`tempMin${measure}`] + finalString;
      currentDiv[1].textContent =
        location.days[index + 1][`temp${measure}`] + finalString;
      currentDiv[2].textContent =
        location.days[index + 1][`tempMax${measure}`] + finalString;
    });
  };
